...
_view_label = tk.Label(ui, borderwidth=3, relief="sunken", text="Calculations here", width=34, bg="#f5d0d0", fg="#000000")
...
def res(s):
    ...
    if stat == "+":
        ...
        num2 = str(num2[9:])
        raw = float(math.pow(fnum, float(1/int(num2))))
        ans = "{:.2f}".format(raw)
    elif stat == "pow":
        num2 = str(num2[10:])
        raw = float(math.pow(fnum, int(num2)))
        ans = "{:.2f}".format(raw)
    e.delete(0, END)
    e.insert(0, ans)
    global flag
    flag = 1
    if stat == "root":
        view(str(num2) + " root of " + str(num) + " = " + str(raw))
    elif stat == "pow":
        view(str(num) + " power " + str(num2) + " = " + str(raw))
    else:
        view(str(num) + " " + str(stat) + " " + str(num2) + " = " + str(raw))
...
sub = Button(ui,activebackground='#ffe46b', bg="#d0c6f5", text="-", padx=18, pady=15, command=lambda: set_operator("-")).grid(row=5, column=3)
mul = Button(ui,activebackground='#ffe46b', bg="#d0c6f5", text="x", padx=17, pady=15, command=lambda: set_operator("x")).grid(row=4, column=3)
div = Button(ui,activebackground='#ffe46b', bg="#d0c6f5", text="/", padx=19, pady=8, command=lambda: set_operator("/")).grid(row=3, column=3)
...
add = Button(ui,activebackground='#ffe46b', bg="#d0c6f5", text="+", padx=15, pady=42, command=lambda: set_operator("+")).grid(row=6, column=3, rowspan=2)
